...
export const handler = async (event) => {
    ...
    try {
        ...
        if ((!headers['origin'].includes("anifie.community.admin") && !headers['origin'].includes("honda-synergy-lab.jp") && !headers['origin'].includes("anifie.com"))
        ...
        let sql = `UPDATE "${tableName}" SET coffee_claim_status = 'UNCLAIM' WHERE PK = '${asset.PK}' AND SK = '${asset.SK}'`;
        ...
        console.error('error in ch-coffee-unclaim-post ' + random10DigitNumber, e);
    
        const message = {
            Subject: 'Honda Cardano Error - ch-coffee-unclaim-post - ' + random10DigitNumber,
            Message: `Error in ch-coffee-unclaim-post: ${e.message}\n\nStack trace:\n${e.stack}`,
