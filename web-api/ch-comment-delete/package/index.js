...
export const handler = async (event) => {
    ...
    try {
        ...
        if(member.user_id !== comment.sender_id) {
            return {
                Success: false,
        ...
        sql = `select * from "${tableName}"."InvertedIndex" where SK = '${'COMMENT#' + comment.comment_id}'`;
        let commentAndLikeResult = await dbClient.send(new ExecuteStatementCommand({ Statement: sql }));
        if(commentAndLikeResult.Items.length > 0) {
            let commentAndLikes = commentAndLikeResult.map(unmarshall);
            for (let i = 0; i < commentAndLikes.length; i++) {
                const commentAndLike = commentAndLikes[i];
                sql = `delete from "${tableName}" where PK = '${commentAndLike.PK}' and SK = '${commentAndLike.SK}'`;
        ...
        console.log("delete comment dbResult", dbTxResult);
        ...
        console.error('error in ch-comment-delete ' + random10DigitNumber, e);
        
        const message = {
            Subject: 'Honda Cardano Error - ch-comment-delete - ' + random10DigitNumber,
            Message: `Error in ch-comment-delete: ${e.message}\n\nStack trace:\n${e.stack}`,
