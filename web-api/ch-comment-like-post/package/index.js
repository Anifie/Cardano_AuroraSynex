...
export const handler = async (event) => {
    ...
    try {
        ...
        if(commentLikeResult.Items.length > 0) {
            console.log("Comment already liked by this user");
            return {
                Success: false,
        ...
        sql = `INSERT INTO "${tableName}" 
                            VALUE {
                                    'PK': 'COMMENTLIKE#${member.user_id}',
                                    'SK': '${'COMMENT#' + comment.comment_id}',
                                    'type': 'COMMENTLIKE',
        ...
        console.log("like comment dbResult", dbTxResult);
        ...
        console.error('error in ch-comment-like-post ' + random10DigitNumber, e);
        
        const message = {
            Subject: 'Honda Cardano Error - ch-comment-like-post - ' + random10DigitNumber,
            Message: `Error in ch-comment-like-post: ${e.message}\n\nStack trace:\n${e.stack}`,
